...
async function lastBlockWorked(_lastBlocknumber){
	var con2 = mysql.createConnection(DB_CONFIG);	
	...
	web31.eth.getTransaction(q).then((z)=>{				
		...
		if(z.to === null){				
			...
			web32.eth.getTransactionReceipt(q).then((x)=>{
				...
				db_insert1(_contractAddress, _blockNumber, _deployer_addr, _transaction_fees_eth, _referrer_wallet='--', q);				
...
async function	db_select(){	
	...
	try{
			return await query("SELECT * FROM "+process.env.DB_SCRIPT_BLOCKS_TABLE+" limit 0,1");					
	}catch(e){
		console.log("ERROR >>Catch",e);
	}finally{
			con.end();			
...
async function	db_delete(_blockNumber){	
	var mycon2 = mysql.createConnection(DB_CONFIG);
